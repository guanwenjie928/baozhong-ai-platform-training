...
#
# 模板占位符（使用前替换为实际值）:
#   {{APP_NAME}}     - 应用简称（英文，用于日志/进程名/nginx配置文件名）
#   {{APP_TITLE}}    - 应用标题（中文，用于输出显示）
#   {{DEFAULT_PATH}} - 默认路径前缀（如 /zhique/）
#
...
# ─── 配置（可通过命令行参数覆盖）──────────────────────────
BASE_PATH="${1:-{{DEFAULT_PATH}}}"     # 前端 base 路径（nginx 分流用）
SERVER_PORT="${2:-9000}"               # FastAPI 期望端口（被占用时自动递增到空闲端口）
NGINX_PORT="${3:-8080}"                # nginx 对外端口
NGINX_CONF="/etc/nginx/conf.d/{{APP_NAME}}.conf"  # 按应用名独立配置文件，避免互相覆盖
...
if [ -d "${PROJECT_DIR}/frontend" ]; then
    ...
    echo ""
    echo "🔧 配置 base: ${BASE_PATH}"

    # 更新 vite.config.ts（React/Vite 项目）
    if [ -f "${PROJECT_DIR}/frontend/vite.config.ts" ]; then
        sed -i "s|base: '.*'|base: '${BASE_PATH}'|" "${PROJECT_DIR}/frontend/vite.config.ts"
    fi

    # 更新 api.ts 中的 BASE 常量（如果 base 路径非空）
    if [ -n "$BASE_PATH" ] && [ "$BASE_PATH" != "/" ]; then
        API_BASE="${BASE_PATH%/}/api"
        if [ -f "${PROJECT_DIR}/frontend/src/api.ts" ]; then
            sed -i "s|const BASE = '.*'|const BASE = '${API_BASE}'|" "${PROJECT_DIR}/frontend/src/api.ts"
        fi
    fi
...
echo ""
echo "🧹 清理 {{APP_TITLE}} 旧实例..."

# 通过进程名精确匹配本应用的旧实例（不依赖端口号，不影响其他应用）
OLD_PIDS=$(pgrep -f "python3.*{{APP_NAME}}" 2>/dev/null || true)
if [ -n "$OLD_PIDS" ]; then
...
server {
    ...
    # {{APP_TITLE}} 服务（${BASE_PATH} → ${SERVER_PORT}）
    location ${BASE_PATH} {
...
NGINX_EOF
        ...
        echo "   独占模式：全量代理到 {{APP_TITLE}} (${SERVER_PORT})"
        cat > "$NGINX_CONF" << NGINX_EOF
server {
    listen ${NGINX_PORT};
    server_name _;
...
NGINX_EOF
    ...
    # 清理可能残留的旧 default.conf（已废弃，统一迁移至按应用名命名的配置文件）
    if [ -f "/etc/nginx/conf.d/default.conf" ]; then
        echo "   清理旧 default.conf（已迁移至 {{APP_NAME}}.conf）"
...
echo ""
echo "🚀 启动 {{APP_TITLE}} 服务 (端口 ${SERVER_PORT})..."
